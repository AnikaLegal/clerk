...
2. Run `just schema` (or `cd frontend && npm run schema`). This bundles the spec into `app/openapi.generated.yaml`, regenerates the RTK Query client at `frontend/src/api/api.generated.ts` (intake endpoints are filtered out via `frontend/openapi-config.js`), and regenerates the intake types at `intake/src/api/types.generated.ts`.

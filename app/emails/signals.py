...
from .models import Email, EmailState, ReceivedEmail
from .service import ingest_email_task, receive_email_task, send_email_task
...
def save_email(sender, instance, **kwargs):
    ...
    if email.state == EmailState.READY_TO_SEND:
        ...
        async_task(ingest_email_task, email.pk)
...
def post_save_received_email(sender, instance, **kwargs):
    email = instance
    transaction.on_commit(lambda: async_task(receive_email_task, email.pk))

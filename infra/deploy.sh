...
# Wait for the services to converge so this fails if Swarm rolls the update back.
docker stack deploy --detach=false --compose-file "docker/docker-compose.${COMPOSE_SUFFIX}.yml" "$PROJECT_NAME"

echo -e "\n>>> Removing unused Docker objects (containers, images, networks, build cache)"
# --all is needed because stack deploy pins images by digest, so a replaced
# image is unused rather than dangling. Volumes are left alone.
